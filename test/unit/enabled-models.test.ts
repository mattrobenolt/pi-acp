...
test('PiAcpAgent: newSession filters advertised models using enabledModels', async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({ quietStartup: true, enabledModels: ['anthropic/claude-sonnet-4-6', 'openai-codex/gpt-5.*'] }),
    'utf-8'
  )
  ...
  try {
    const session = {
      ...
      proc: {
        ...
        async getState() {
          return { thinkingLevel: 'medium', model: { provider: 'anthropic', id: 'claude-sonnet-4-6' } }
        },
        async getCommands() {
          return { commands: [] }
        }
